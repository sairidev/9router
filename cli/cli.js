...
for (let i = 0; i < args.length; i++) {
  if (args[i] === "--port" || args[i] === "-p") {
    ...
    if (parsedPort > 0) { port = parsedPort; portFromArg = true; }
    i++;
  } else if (args[i] === "--host" || args[i] === "-H") {
    host = args[i + 1] || DEFAULT_HOST;
    i++;
  } else if (args[i] === "--password" || args[i] === "-P") {
    // Sets INITIAL_PASSWORD for the dashboard (needed for remote login on a fresh install)
    if (args[i + 1]) process.env.INITIAL_PASSWORD = args[i + 1];
    i++;
...
Options:
  ...
  -P, --password <pw> Dashboard password (sets INITIAL_PASSWORD; required for remote login
                      on a fresh install, otherwise the default 123456 is blocked)
  -n, --no-browser    Don't open browser automatically
  -l, --log           Show server logs (default: hidden)
  -t, --tray          Run in system tray mode (background)
  --skip-update       Skip auto-update check
  --simple-menu       Numbered text menus (for Pterodactyl / consoles without arrow keys).
                      Auto-enabled when stdin is not a TTY or a Pterodactyl container is detected.
                      Env: NINEROUTER_SIMPLE_MENU=1 (force on) / 0 (force off)
...
// Pterodactyl: the panel's startup variable is not always expanded (e.g. a literal
// "{{SERVER_PORT}}" ends up in --port), so fall back to the allocation port that
// Wings exposes as the SERVER_PORT env var instead of the 20128 default.
if (!portFromArg && (process.env.P_SERVER_UUID || process.env.P_SERVER_LOCATION)) {
  const allocPort = parseInt(process.env.SERVER_PORT, 10);
...
function checkForUpdate() {
  return new Promise((resolve) => {
    if (skipUpdate || process.env.NINEROUTER_NO_UPDATE_CHECK === "1") {
      resolve(null);
      return;
    }
